...
export default function GoalsPage() {
  ...
  const loadGoals = async (userId: string) => {
    ...
    try {
      ...
      if (res.ok) {
        // Remove duplicates by goal id.  Without deduplication, goals can appear
        // multiple times if they are created more than once on the backend.
        // Deduplicate by name rather than ID.  Goals can have different IDs
        // (e.g. when created multiple times) but identical names.  Filter by
        // lower‑cased name to ensure uniqueness.
  ...
  const createDefaultGoals = async (userId: string) => {
    try {
      ...
      if (checkData.exists) {
        ...
        const data = await res.json();
        if (res.ok) {
          // Deduplicate goals by name when reloading after existence check
          const seenNames = new Set<string>();
          const uniqueGoals: any[] = [];
          for (const goal of data.goals) {
            const key = (goal.name || '').trim().toLowerCase();
